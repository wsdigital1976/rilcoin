...
BOOST_AUTO_TEST_CASE(coin_selection_tests)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        // test randomness
        {
            ...
            BOOST_CHECK(testWallet.SelectCoinsMinConf(0 * COIN, 1, 6, 0, vCoins, setCoinsRet , nValueRet));
            BOOST_CHECK(testWallet.SelectCoinsMinConf(0 * COIN, 1, 6, 0, vCoins, setCoinsRet2, nValueRet));
...
BOOST_FIXTURE_TEST_CASE(rescan, TestChain100Setup)
{
    ...
    // file.
    {
        ...
        BOOST_CHECK_EQUAL(wallet.GetImmatureBalance(), 0 * COIN);
    }
    ...
    // after.
    {
        ...
        BOOST_CHECK_EQUAL(response.write(),
            strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":\"Rescan failed for key with creation "
                      ...
                      "by pruning or data corruption (see rilcoind log for details) and could be dealt with by "
...
BOOST_FIXTURE_TEST_CASE(ListCoins, ListCoinsTestingSetup)
{
    ...
    BOOST_CHECK_EQUAL(0 * COIN, wallet->GetAvailableBalance());
