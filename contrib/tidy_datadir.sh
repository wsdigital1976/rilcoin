...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Rilcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Rilcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Rilcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Rilcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Rilcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Rilcoin 0.8 datadir."
    ;;
esac
